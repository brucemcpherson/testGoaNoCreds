...
function doGetShoeBoxed (e) {
  ...
  var goa = cGoa.GoaApp.createGoa('shoeboxed',PropertiesService.getScriptProperties()).execute(e);
  ...
  var result = testShoeBoxed (goa.getToken(), goa.getParams() );   
...
function shoeBoxed(params) {
       
  // pick up the token refreshing if necessary
  var goa = cGoa.GoaApp.createGoa('shoeboxed', PropertiesService.getScriptProperties()).execute(params);
  ...
  Logger.log (testShoeBoxed (goa.getToken(), goa.getParams() ));
 ...
 */
function testShoeBoxed (accessToken,params) {
   ...
   return  UrlFetchApp.fetch("https://api.shoeboxed.com/v2/user", options);
